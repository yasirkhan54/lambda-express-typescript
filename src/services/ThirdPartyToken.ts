...
export const CREATE_THIRDPARTYTOKEN = async (thirdPartyTokenValue: ThirdPartyToken) => {
  const thirdPartyToken = await SQL<ThirdPartyToken[]>`INSERT INTO ${SQL(DATABASE_TABLES.THIRD_PARTY_TOKENS)} VALUES ${SQL(thirdPartyTokenValue)} returning *` // Create thirdPartyToken
