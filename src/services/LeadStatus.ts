...
export const CREATE_LEADSTATUS = async (leadStatus: LeadStatus) => {
  const leadStatuses = await SQL`INSERT INTO ${SQL(DATABASE_TABLES.LEAD_STATUS)} ${SQL(leadStatus, 'lead_id', 'api_response_json', 'original_status', 'current_status', 'status_update_timestamp', 'reject_reason', 'type')} returning *` // Create lead status
  if (leadStatuses.length === 0) {
