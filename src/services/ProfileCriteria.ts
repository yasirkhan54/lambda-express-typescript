...
export const CREATE_PROFILECRITERIA = async (profileCriteriaValues: ProfileCriteria) => {
  const profileCriteria = await SQL`INSERT INTO ${SQL(DATABASE_TABLES.PROFILE_CRITERIA)} ${SQL(profileCriteriaValues, 'lead_id', 'age', 'gender', 'is_us_citizen', 'is_us_military', 'military_status', 'military_branch', 'high_school_graduation_year')} returning *` // Create profileCriteria
  if (profileCriteria.length === 0) {
