...
export const CREATE_CONSENT = async (consent: Consent) => {
  const consents = await SQL`INSERT INTO ${SQL(DATABASE_TABLES.CONSENTS)} ${SQL(consent, 'lead_id', 'tcpa_timestamp_traffic', 'tcpa_timestamp_marketing', 'tcpa_text_traffic', 'tcpa_timestamp_client')} returning *` // Create consent
  if (consents.length === 0) {
