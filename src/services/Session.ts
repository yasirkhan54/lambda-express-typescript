...
export const CREATE_SESSION = async (session: Session) => {
  const sessions = await SQL`INSERT INTO ${SQL(DATABASE_TABLES.SESSIONS)} ${SQL(session, 'lead_id', 'ip', 'web_url', 'traffic_brand_name', 'traffic_source_detail', 'traffic_source_type', 'web_access_key', 'web_session_id', 'landing_page', 'device_type')} returning *` // Create session
  if (sessions.length === 0) {
